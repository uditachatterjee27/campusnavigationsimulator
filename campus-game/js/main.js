...
function addStreetLamp(scene, x, z, height = 6) {
  ...
  // Keep light subtle (too many point lights can hurt performance)
...
function init3D() {
  ...
  // trees around edges
  addTree(scene, -55, -55, 1.25);
  addTree(scene, -65, -20, 1.05);
  addTree(scene, -62, 40, 1.15);
  addTree(scene, 55, -55, 1.2);
  addTree(scene, 65, -18, 1.05);
  addTree(scene, 62, 42, 1.2);
  addTree(scene, 0, 65, 1.3);
  addTree(scene, 0, -65, 1.3);
  ...
  function makeBuilding(name, x, z, w = 7, d = 7, h = 8) {
    // Requested tweaks:
    // - Boyd Orr & Library: taller (not wider)
    // - Fraser: flatter but wider
    ...
    // Main selectable mesh (ONLY this goes into `buildings[]`)
    ...
    // WINDOWS: smaller repeated windows (not one massive plane)
